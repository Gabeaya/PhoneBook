using System;
using System.Collections.Generic; // a namespace built within .NET allows C# to use Dictionar Objects(required when using dictionsaries)
...
class PhoneBook
{
  ...
  static void AddContact()
  {
    ...
    if (phoneNumbers.ContainsKey(name)) //checks to see if our phone numbers object conatains the added name to prevent duplicates
    {
  ...
  static void LookUpContact()
  {
    ...
    {
      string value = phoneNumbers[friend];//calls on the variable equal to the user input and checks it as an element in the index of phone numbers object
